...
const get = async <T>(url: string, config?: AxiosRequestConfig): Promise<T | ServerError> => {
...
const doDelete = async <T>(url: string, config?: AxiosRequestConfig): Promise<T | ServerError> => {
...
export const getBuckets = (storageType: string): Promise<string[] | ServerError> =>
  get(`/storage/buckets/${storageType}`);
...
export const getMediaThumbnailUrl = (mf: MediaFile) =>
  `${baseUrl}/media/thumbnail/png/100/${mf.id}/${mf.path}`;

export const getMediaDownloadUrl = (mf: MediaFile) =>
  `${baseUrl}/media/download/${mf.id}/${mf.path}`;

export const deleteMediaFile = (id: number): Promise<{ [id: string]: string } | ServerError> =>
  doDelete(`/media/${id}`);

export const createBucket = (name: string): Promise<string[] | ServerError> =>
  post(`/storage/bucket/${name}`, null);

export const deleteBucket = (name: string): Promise<BucketData | ServerError> =>
  doDelete(`/storage/bucket/${name}`);
